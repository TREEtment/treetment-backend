...
public class CareerController {
    ...
    // 2. 특정 상담사의 모든 경력 조회 API (GET /api/careers/counselor/{counselorId})
    ...
    // 3. 경력 내용 수정 API (PUT /api/careers/{careerId})
    @PutMapping("/{careerId}")
    public ResponseEntity<CareerResponseDTO> updateCareer(@PathVariable Long careerId, @RequestBody Map<String, String> requestBody) {
        // 요청 본문에서 "careerContent" 키의 값을 가져옵니다.
    ...
    // 4. 경력 삭제 API (DELETE /api/careers/{careerId})
